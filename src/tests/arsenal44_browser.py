...
with sync_playwright() as pw:
 ...
 try:
  ...
  if not a.local_only:
   ...
   check(host.locator('#cooldownText1').inner_text()=='READY','Continuous-fire feedback has no false cooldown')
   check(host.locator('#machineBudget1').is_visible() and host.locator('#machineBudget1').inner_text().startswith('FIRE '),'Remaining machine-gun firing time appears beside ammo')
