...
test('unchanged room metadata does not rebuild the field manual DOM',()=>{
 ...
 assert.equal(b.$('manualContent').htmlWrites,1);assert.equal(b.$('rulesSummary').htmlWrites,1);assert.match(b.$('rulesSummary').innerHTML,/<span class="room-rule-item">FRIENDLY FIRE OFF<\/span>/);
...
test('local roster difficulty edits ignore inherited object names',()=>{
 const b=boot();b.s.localRoom.players=players(2);const bot=b.s.localRoom.players[1];
 for(const difficulty of ['constructor','toString','__proto__']){b.s.changeSeat(bot,{difficulty});assert.equal(bot.difficulty,'normal');}
 b.s.changeSeat(bot,{difficulty:'hard'});assert.equal(bot.difficulty,'hard');
});
