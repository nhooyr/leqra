"""v3.5: live production handler checks. Run python tests/features35_protocol.py http://127.0.0.1:8080."""
...
async def main():
 ...
 try:
  cfg=json.load(urlopen(wire.BASE+'/api/config'));health=json.load(urlopen(wire.BASE+'/healthz'));check(health['version']=='3.5.0','Production reports version 3.5.0');check(cfg['maxPlayers']==8,'API advertises eight tank seats')
  ...
  r=await op(host,'rules',rules=rules);check(r['rules']['mapSize']=='huge','Host can choose 14x12')
  ...
  report={'version':'3.5.0','passed':len(checks),'checks':checks,'handler':'Production HTTP/WebSocket with normal origin validation; no fixture endpoints.'};(Path(__file__).parent/'results/features35-protocol.json').write_text(json.dumps(report,indent=2));print('TOTAL',len(checks))
