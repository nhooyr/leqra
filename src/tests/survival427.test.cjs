...
function clearWave(s){if(s.phase==='countdown')s.update(3);for(const t of s.tanks)if(t.survivalEnemy)t.alive=false;s.stepLocalSurvival(1/120);}
...
test('wave clear awards every squad member once, then starts a new maze countdown after two seconds',()=>{
 ...
 assert.equal(s.survivalState().status,'break');assert.equal(s.tanks[0].alive,false);assert.equal(s.bullets.length,0);assert.equal(s.pickups.length,1);assert.equal(s.pickups[0].life,4);assert.deepEqual(Array.from(s.scores.slice(0,2)),[1,1]);assert.equal(s.localRoom.players,players);s.stepLocalSurvival(1.9);assert.equal(s.survivalState().wave,1);s.stepLocalSurvival(.1);
 assert.equal(s.survivalState().wave,2);assert.equal(s.survivalState().status,'wave');assert.notEqual(s.grid,grid);assert.equal(s.phase,'countdown');assert.equal(s.phaseTime,3);assert.equal(s.roundClock,75);assert.equal(s.tanks.filter(t=>!t.survivalEnemy).every(t=>t.alive&&t.power===null),true);assert.equal(s.seeds,1);assert.ok(s.inputClears>=3);
});
...
test('wave breaks preserve machine firing budget, then revival clears it',()=>{
 ...
 assert.equal(s.fire(pilot),false);assert.equal(pilot.machineRounds,73);s.stepLocalSurvival(2);
...
test('Huge survival bosses receive fifteen-second buffs and dead bosses leave the HUD',()=>{
 ...
 s.update(3);boss.alive=false;s.stepLocalSurvival(1/120);assert.equal(s.survivalState().boss,false);assert.equal(s.survivalState().enemiesRemaining,3);assert.equal(s.phase,'playing');
});
test('the earned wave-break revival continues if the surviving pilot spectates',()=>{
 ...
 s.localRoom.players[0].spectating=true;s.tanks=s.tanks.filter(t=>t.id!==0);s.stepLocalSurvival(2);
 assert.equal(s.phase,'countdown');assert.equal(s.survivalState().wave,2);assert.equal(s.tanks.find(t=>t.id===1).alive,true);
});
test('generated enemies cannot inherit old scores and finished runs preserve scene pickups',()=>{
 ...
 s.update(3);s.pickups=[{life:30}];s.endLocalSurvival(false);assert.equal(s.pickups.length,1);assert.equal(s.pickups[0].life,30);
});
...
test('defeated local enemies retain their lineup rows until the next wave replaces them',()=>{
 const s=boot(),prior=s.tanks.filter(t=>t.survivalEnemy);for(const name of ['shieldCount','grantLocalSurvivalSquadShields','botLevelName','lineupMember','scoreboardEntries'])vm.runInContext(declaration(name),s);
 clearWave(s);s.stepLocalSurvival(1.9);
...
test('all-bot local squads clear waves, revive and win while their human host spectates',()=>{
 ...
 assert.equal(s.roomStartError(),'');s.tanks[1].alive=false;clearWave(s);assert.equal(s.survivalState().status,'break');s.stepLocalSurvival(2);assert.equal(s.survivalState().wave,2);assert.equal(s.tanks[1].alive,true);clearWave(s);
...
test('each local Survival wave protects its respawned squad with the real spawn duration',()=>{
 const s=boot(),first=s.tanks.find(t=>t.human);assert.equal(first.spawnProtected,true);assert.equal(first.invulnerable,.75);
 s.fxTime=20;clearWave(s);s.stepLocalSurvival(2);const next=s.tanks.find(t=>t.human);assert.equal(s.localObjectives.survival.wave,2);assert.equal(next.spawnProtected,true);assert.equal(next.invulnerable,1.2);
 s.phase='paused';assert.equal(s.restartLocalSurvivalWave(),true);assert.equal(s.tanks.find(t=>t.human).spawnProtected,true);assert.equal(s.tanks.find(t=>t.human).invulnerable,1.2);
});

test('a lost Survival boss wave retries on the same maze with the prior-wave score and statistics',()=>{
 const s=boot({target:15});
 for(let wave=1;wave<5;wave++){s.localMatchStats.rows[0].eliminations+=2;s.localMatchStats.duration+=12;clearWave(s);s.stepLocalSurvival(2);}
 s.update(3);const maze=s.grid,earned=s.localMatchStats.rows[0].eliminations,duration=s.localMatchStats.duration,roster=s.localRoom.players,boss=s.tanks.find(t=>t.survivalBoss);
 ...
 assert.equal(s.restartLocalSurvivalWave(),true);assert.equal(s.phase,'countdown');assert.equal(s.phaseTime,3);assert.equal(s.roundClock,75);assert.equal(s.grid,maze);assert.equal(s.localRoom.players,roster);
...
test('repeated failed attempts cannot farm statistics or advance the Survival wave',()=>{
 const s=boot({target:2});clearWave(s);s.stepLocalSurvival(2);
...
test('Survival restart eligibility rejects breaks, empty squads and stale lobby or mode state',()=>{
 const s=boot();assert.equal(s.canRestartSurvivalWave(),true);clearWave(s);assert.equal(s.restartLocalSurvivalWave(),false);s.stepLocalSurvival(2);s.update(3);s.endLocalSurvival(false);assert.equal(s.canRestartSurvivalWave(),true);
...
test('every available local squad tank starts boss waves shielded even with shield pickups disabled',()=>{
 for(const pickupRate of ['superfast','off']){
  const s=boot({count:4,target:20,weapons:['laser'],pickupRate});s.localRoom.players[1].kind='local';s.localRoom.players[3].connected=false;
  assert.ok(s.tanks.filter(t=>!t.survivalEnemy).every(t=>s.shieldCount(t)===0),'ordinary first wave gives no starting shields');
  for(const wave of [5,10,15,20]){
   s.localObjectives.survival.wave=wave-1;s.startLocalSurvivalWave();
   const allies=s.tanks.filter(t=>!t.survivalEnemy);assert.equal(allies.length,3);
   for(const t of allies){assert.equal(t.alive,true);assert.equal(s.shieldCount(t),1);assert.equal(t.shield,10);}
   assert.ok(s.tanks.filter(t=>t.survivalEnemy).every(t=>s.shieldCount(t)===0),'squad bonus must not equip enemies');
  }
 }
});

test('current and lost boss-wave retries restore the full shield bonus to an all-bot squad',()=>{
 const s=boot({count:3,pickupRate:'off',weapons:['laser']});s.cols=14;s.rows=12;s.localRoom.players[0].spectating=true;s.localRoom.players.slice(1).forEach(p=>p.kind='bot');
 s.localObjectives.survival.wave=4;s.startLocalSurvivalWave();assert.equal(s.tanks.filter(t=>!t.survivalEnemy).length,2);
 for(const lost of [false,true]){
  for(const t of s.tanks)if(!t.survivalEnemy){t.shield=0;t.shieldCharges=0;}
  if(lost)s.endLocalSurvival(false);else{s.phase='paused';s.pausedFrom='playing';}
  assert.equal(s.restartLocalSurvivalWave(),true);
  for(const t of s.tanks.filter(t=>!t.survivalEnemy)){assert.equal(s.shieldCount(t),1);assert.equal(t.shield,15);}
 }
 s.phase='playing';clearWave(s);s.stepLocalSurvival(2);assert.equal(s.survivalState().wave,6);
 assert.ok(s.tanks.filter(t=>!t.survivalEnemy).every(t=>s.shieldCount(t)===0),'next ordinary wave clears the boss bonus');
});

test('boss protection preserves stronger shields and skips dead, spectating and disconnected tanks',()=>{
 const s=boot({count:4});const [strong,expired,spectating,offline]=s.tanks;
 strong.shield=7;strong.shieldCharges=3;expired.shield=0;expired.shieldCharges=4;
 s.localRoom.players[2].spectating=true;s.localRoom.players[3].connected=false;s.grantLocalSurvivalSquadShields();
 assert.equal(strong.shield,7);assert.equal(s.shieldCount(strong),3);assert.equal(expired.shield,10);assert.equal(s.shieldCount(expired),1);
 assert.equal(s.shieldCount(spectating),0);assert.equal(s.shieldCount(offline),0);assert.ok(s.tanks.filter(t=>t.survivalEnemy).every(t=>s.shieldCount(t)===0));
 expired.shield=0;expired.shieldCharges=0;expired.alive=false;s.grantLocalSurvivalSquadShields();assert.equal(s.shieldCount(expired),0);assert.equal(expired.alive,false);
});

test('completed Survival waves freeze nearly expired pickups until the next wave replaces them',()=>{
 const s=boot(),pickup={type:'shield',x:s.tanks[0].x,y:s.tanks[0].y,age:29.95,life:.05},before={...pickup};
 s.pickups=[pickup];Object.assign(s,{fxTime:0,time:0,phaseTime:0,uiClock:1});vm.runInContext(declaration('update'),s);clearWave(s);
 for(let tick=0;tick<119;tick++){
  s.update(1/60);assert.equal(s.pickups.length,1);assert.equal(s.pickups[0],pickup);assert.deepEqual(pickup,before);
 }
 assert.equal(s.survivalState().wave,1);s.update(1/60);assert.equal(s.survivalState().wave,2);assert.equal(s.pickups.includes(pickup),false);
});
test('finished Survival scenes freeze pickups throughout the result reveal',()=>{
 for(const won of [false,true]){
  const s=boot({target:1}),pickup={type:'shield',x:s.tanks[0].x,y:s.tanks[0].y,age:29.95,life:.05},before={...pickup};
  s.pickups=[pickup];Object.assign(s,{fxTime:0,time:0,phaseTime:0,uiClock:1});vm.runInContext(declaration('update'),s);if(won)clearWave(s);else s.endLocalSurvival(false);
  for(let tick=0;tick<180;tick++)s.update(1/60);
  assert.equal(s.pickups.length,1);assert.equal(s.pickups[0],pickup);assert.deepEqual(pickup,before);
 }
});
test('Survival holds its cleared maze for 240 ticks, then freezes a fresh boss arena for a full 360-tick countdown',()=>{
 const s=boot({target:15});s.localObjectives.survival.wave=3;s.startLocalSurvivalWave();s.update(3);
 const state=s.survivalState(),maze=s.grid,stats=s.localMatchStats;s.localMatchStats.duration=12;
 clearWave(s);assert.equal(state.wave,4);assert.equal(state.status,'break');
 for(let tick=1;tick<240;tick++){s.update(1/120);assert.equal(s.grid,maze);assert.equal(state.status,'break');assert.equal(state.wave,4);}
 s.update(1/120);assert.equal(s.phase,'countdown');assert.equal(s.phaseTime,3);assert.notEqual(s.grid,maze);assert.equal(state.wave,5);assert.equal(state.wavesCleared,1);assert.equal(s.localMatchStats,stats);
 const newMaze=s.grid,boss=s.tanks.find(t=>t.survivalBoss),squad=s.tanks.filter(t=>!t.survivalEnemy),frozen=JSON.stringify(s.tanks),reportDuration=stats.duration;
 assert.equal(boss.power,'homing');assert.equal(boss.shieldCharges,1);assert.ok(squad.every(t=>t.shieldCharges>=1));
