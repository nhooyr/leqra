...
Leave room now uses the same neon-red outline color as REMOVE/KICK. New tanks appear at the bottom of room rosters, independent of their reusable gameplay seat numbers; Add Bot inherits the last roster bot’s difficulty. A recognized recent saved session reconnects automatically, preserving the server’s callsign and player/spectator role. New invite visitors still choose a callsign and press JOIN. Expired or kicked memberships also require explicit JOIN. The organized source tree and earlier maze stability fixes are retained.

See [UPDATE-v4.45.0.md](releases/UPDATE-v4.45.0.md) for release changes and [TEST-NOTES-v4.45.0.md](releases/TEST-NOTES-v4.45.0.md) for verification. Earlier versioned guides remain available in [releases/](releases/) as historical records; use the commands in this guide for the current repository layout. See the [changelog](../CHANGELOG.md) for the complete release history.

## Repository layout

Run all setup, build, test and deployment commands below from the repository root, the directory containing `go.mod`.

| Path | Contents |
| --- | --- |
| `src/` | Go server sources and Go tests |
| `src/web/` | Embedded browser game and versioned production assets |
| `src/tests/` | JavaScript tests, optional browser/protocol harnesses and recorded results |
| `scripts/` | Server launchers and maintenance utilities |
| `deploy/` | Dockerfile, Compose configuration and reverse proxy example |
| `docs/` | Setup, protocol, testing and feature guides |
| `docs/releases/` | Historical release notes and verification reports |
| `bin/` | Local build output; generated and ignored by Git |
...
The static deployment is now version-safe. `index.html` is always revalidated, and every other browser asset served by Go lives under **`/assets/v4.45.0/`** and is sent with an immutable one-year cache policy. A future release therefore gets a new asset URL instead of waiting for an old JavaScript/CSS cache entry to expire. The service worker follows the same versioned shell and removes older `leqra-app-*` caches after activation.
...
The Safari audio fixes, game-styled confirmations and remappable alternate fire keys from v4.26 are retained. See [UPDATE-v4.26.0.md](releases/UPDATE-v4.26.0.md) for that release and its physical-device testing limits; the current changes and verification are in [UPDATE-v4.45.0.md](releases/UPDATE-v4.45.0.md) and [TEST-NOTES-v4.45.0.md](releases/TEST-NOTES-v4.45.0.md).
...
statistics remain intact. See [UPDATE-v4.45.0.md](releases/UPDATE-v4.45.0.md) for current behavior and installation and [TEST-NOTES-v4.45.0.md](releases/TEST-NOTES-v4.45.0.md) for current verification. Older weapon guides describe the releases named in their headings; the power-up table below gives current timing.
...
not a promise of zero-latency server-confirmed online hits. See [UPDATE-v4.2.md](releases/UPDATE-v4.2.md)
for behavior and installation, and [TEST-NOTES-v4.2.md](releases/TEST-NOTES-v4.2.md) for measurements and limits.
...
See [POWERUPS.md](POWERUPS.md) and the retained [UPDATE-v4.1.md](releases/UPDATE-v4.1.md) for weapon and expiry rules.
...
See [UPDATE-v4.0.md](releases/UPDATE-v4.0.md) for queue sizes, fixed rules, consent, cancellation, return,
reconnection and deployment boundaries. [TESTING.md](TESTING.md) lists current executions;
...
with you plus three Normal bots. A fresh desktop setup uses Free-for-all on a 12×10 map; phones start with Compact, and saved room rules are restored. Press **PLAY** or change the roster.
Local play works without a WebSocket connection; `src/web/index.html` can also be
...
and enables **COPY INVITE LINK**. Opening that link automatically restores a recognized recent session. New visitors see the join menu with the room prefilled, choose a callsign and press **JOIN**.
...
Go is required on the hosting computer only. `scripts\run.bat` (Windows) or `sh scripts/run.sh`
...
## Saved rules and roster

The browser remembers each game mode’s rules and the last roster setup across runs and reloads, including tank names, bot levels, teams, FFA colors, local Player 2 and spectating. Switching modes restores its remembered rules while keeping the current map size and a legal battle format. In the Rules menu, **DEFAULT RULES** loads that mode’s defaults into the form; **APPLY RULES** saves them. Local settings and accepted private-host settings are remembered; remote guest tanks and matchmaking rosters are omitted.
...
are retained as historical notes; current behavior is described in this README and [UPDATE-v4.45.0.md](releases/UPDATE-v4.45.0.md), with the retained
...
on restart. Both the health endpoint and browser version should show **4.45.0**.
...
```powershell
go build -trimpath -o bin/leqra.exe ./src
.\bin\leqra.exe
```

The executable embeds `src/web/`, so it can run without separate assets. **Rebuild it after changing the web files.** The ZIP intentionally contains source rather than unsigned prebuilt executables.

Use a different port:

```sh
go run ./src -addr :9090
```
...
```sh
docker build -f deploy/Dockerfile -t leqra .
docker run --rm -p 8080:8080 leqra
# Or:
...
`src/matchmaking.go` runs consented human-only queues and preserves original private lobbies. It creates separate fixed-rule battle rooms, then uses the existing game engine.

`src/game.go` owns the online simulation, with `src/bots.go` providing per-bot AI and `src/roster.go` validating room setup: movement, collision detection, bounces, damage, cooldowns, ammunition, power-ups, rounds, and scores. It advances at **60 simulation ticks/second**. `src/hub.go` sends **30 state snapshots/second** and sends the maze when a new generation starts or a client reconnects.
...
See [TESTING.md](TESTING.md) for the actual checks performed, optional live WebSocket/browser scripts, and test limitations. [PROTOCOL.md](PROTOCOL.md) describes messages and state ownership.

| Source | Responsibility |
| --- | --- |
| `src/main.go` | HTTP server, embedded assets and configuration |
| `src/transport.go` | Bounded WebSocket framing and handshake |
| `src/hub.go` | Rooms, sessions, validation and broadcasts |
| `src/matchmaking.go` | Party consent, queues, team packing and room transfers |
| `src/game.go` | Authoritative online tank simulation |
| `src/survival.go` | Cooperative waves, boss loadouts and squad lifecycle |
| `src/bots.go` | Server bot navigation, aiming and evasion |
| `src/roster.go` | Host-only roster setup, sharing and team rules |
| `src/teams.go` | Unicode team names and rule copying |
| `src/web/` | Canvas game, offline AI, online client and responsive controls |
| `src/web/netcode.js` | Prediction, reconciliation and snapshot playback math |
| `src/*_test.go` | Unit, simulation, security and transport tests |
| `src/tests/` | JavaScript tests and optional protocol/browser integration tests |
