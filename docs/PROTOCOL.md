# leqra v4.45.1 — current game protocol (protocol 1)

Deploy the server and complete browser assets together. The JSON framing protocol remains **1**, and the application-version handshake now requires **4.45.1**:

```json
{"type":"server_hello","version":"4.45.1","protocol":1}
{"type":"client_hello","version":"4.45.1","protocol":1}
```
...
Bounds and objective details are in [GAMEPLAY-v3.2.md](releases/GAMEPLAY-v3.2.md) and `src/rules.go`. CTF requires
