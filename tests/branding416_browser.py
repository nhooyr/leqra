"""v4.16 focused lowercase leqra rebrand and storage-migration checks."""
import argparse,json,re
from pathlib import Path
from playwright.sync_api import sync_playwright

ap=argparse.ArgumentParser();ap.add_argument('--output',default='tests/results/v4.16-branding');a=ap.parse_args()
...
with sync_playwright() as pw:
 ...
 p.add_script_tag(content=js);p.wait_for_function("window.__test&&window.leqra&&leqra.version==='4.16.0'")
 ...
 check(p.evaluate("typeof leqra==='object'&&leqra.version==='4.16.0'"),'canonical public browser API is leqra v4.16.0')
 ...
 check(p.locator('.game-version').inner_text().strip()=='leqra v4.16.0','Controls version footer uses lowercase leqra')
 check(not errors,'no uncaught browser errors: '+str(errors))
 p.screenshot(path=str(out/'leqra-v416-branding.png'));c.close();b.close()
(out/'results.json').write_text(json.dumps({'version':'4.16.0','passed':len(checks),'checks':checks,'errors':errors,'migrated':migrated},indent=2));print('TOTAL',len(checks))
