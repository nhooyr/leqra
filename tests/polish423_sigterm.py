...
async def run_socket(base, proc):
    ...
    async with connect(ws_url, origin=base, compression=None, max_size=1_000_000) as ws:
        ...
        check(hello.get('type') == 'server_hello' and hello.get('version') == '4.23.2' and hello.get('protocol') == 1,
        ...
        try:
            for _ in range(80):
                try:
                    ...
                    if data.get('version') == '4.23.2':
                        break
                except Exception:
                    await asyncio.sleep(.05)
            else:
                raise AssertionError('server did not become healthy')
            check(True, 'Compiled production server reports v4.23.2 healthy')
