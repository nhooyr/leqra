"""v4.29.0 mobile-default and supplemental-fire regressions."""
import json,re
from pathlib import Path
from playwright.sync_api import sync_playwright
root=Path(__file__).resolve().parents[1];web=root/'web';out=root/'tests/results/v4.29.0-polish';out.mkdir(parents=True,exist_ok=True)
...
(out/'results.json').write_text(json.dumps({'version':'4.29.0','passed':len(checks),'checks':checks,'errors':errors},indent=2));print('TOTAL',len(checks))
