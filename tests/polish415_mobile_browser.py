...
with sync_playwright() as pw:
 ...
 p.add_script_tag(content=js);p.wait_for_function("window.__test&&leqra.version==='4.33.0'");p.evaluate('__test.createLocalRoom()');p.wait_for_timeout(20)
...
(out/'results.json').write_text(json.dumps({'version':'4.33.0','passed':len(checks),'checks':checks,'errors':errors,'metrics':metrics},indent=2));print('TOTAL',len(checks))
