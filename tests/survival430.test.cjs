...
test('boss preview hides during pause, resumes with the same timer, and disappears at wave start',()=>{
 const s=boot();s.updateAnnouncer();s.phase='paused';s.updateAnnouncer();assert.equal(s.$('announcer').hidden,true);assert.equal(s.$('announcer').classes.has('boss-preview'),false);assert.equal(s.survivalState().breakTime,4);
 s.phase='playing';s.updateAnnouncer();assert.equal(s.$('announcer').hidden,false);assert.equal(s.$('announceMain').textContent,'4');
