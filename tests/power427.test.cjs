...
test('local machine gun fires exactly 300 rapid projectiles at 60 Hz on a 120 Hz simulation',()=>{
 ...
 assert.equal(fired,300);assert.equal(t.machineRounds,0);assert.equal(t.power,null);assert.equal(t.powerTime,0);assert.equal(t.charges,0);
});
test('local cooldown, wall, slot and same-tick blocks never spend firing budget',()=>{
 ...
 s.bullets=Array.from({length:96},()=>({owner:0}));assert.equal(s.fire(t),false);assert.equal(t.machineRounds,300);
 s.bullets=[];assert.equal(s.fire(t),true);assert.equal(s.fire(t),false);assert.equal(t.machineRounds,299);
});
test('local released fire preserves budget but the regular timer still expires unused guns',()=>{
 for(const [cols,rows,duration]of [[12,10,10],[14,12,15]]){
  ...
  assert.equal(t.power,'rapid');assert.equal(t.machineRounds,300);
  s.phase='paused';s.update(3);assert.equal(t.machineRounds,300);assert.ok(t.powerTime>0);
...
test('machine-gun pickup refreshes its firing budget; other buffs preserve it',()=>{
 const s=boot(),t=tank();s.grantPower(t,'rapid');s.fire(t);s.grantPower(t,'shield');assert.equal(t.machineRounds,299);
 s.grantPower(t,'rapid');assert.equal(t.machineRounds,300);assert.equal(t.shieldCharges,1);
...
const onlineTank=id=>({...tank(id),spawnSerial:1,shotSerial:0,power:'rapid',machineRounds:300});
test('online predicted fire reserves remaining rounds and never shows a 301st machine-gun shot',()=>{
 ...
 for(let i=0;i<300;i++){
  ...
  assert.equal(p.machineRounds(t),299-i);assert.equal(t.machineRounds,300-i,'prediction leaves server state intact');
  t.shotSerial=v.shot;t.machineRounds--;p.sync(t,'playing',now+1);p.prune(now+1,new Set([0]));assert.equal(p.machineRounds(t),299-i);
 }
...
test('predicted firing budget is independent for P2 and resets with a new life',()=>{
 ...
 a.spawnSerial++;a.machineRounds=300;p.sync(a,'playing',100);assert.equal(p.machineRounds(a),300);
});
test('pickup feedback explains map duration separately from machine-gun firing time and grenade fuse',()=>{
 ...
 assert.match(s.pickupMessage('rapid'),/5s FIRING · 15s TO USE/);assert.match(s.pickupMessage('grenade'),/10s FUSE/);
});
