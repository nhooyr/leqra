...
def install(page,name='ALPHA',disable_ua_data=False):
 ...
 page.add_script_tag(content=js);page.wait_for_function("window.__test&&leqra.version==='4.30.0'")
...
(out/'results.json').write_text(json.dumps({'version':'4.30.0','passed':len(checks),'checks':checks,'errors':errors,'manual':manual,'arena':{'before':before,'during':during,'after':after},'down':down,'result':result},indent=2));print('TOTAL',len(checks))
