...
(out/'results.json').write_text(json.dumps({'version':'4.23.1','passed':len(checks),'checks':checks,'errors':errors,'manual':manual,'arena':{'before':before,'during':during,'after':after},'down':down,'result':result},indent=2));print('TOTAL',len(checks))
