...
with sync_playwright() as pw:
    ...
    p.add_script_tag(content=(web/'theme.js').read_text());p.add_script_tag(content=(web/'netcode.js').read_text());p.add_script_tag(content=js);p.wait_for_function("window.__test&&leqra.version==='4.22.0'")
...
report={'version':'4.22.0','passed':len(checks),'checks':checks,'errors':errors};(out/'results.json').write_text(json.dumps(report,indent=2));print('TOTAL',len(checks))
