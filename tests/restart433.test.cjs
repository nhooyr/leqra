...
function boot(){
 ...
 const $=id=>elements.get(id)||null;doc={body:new Element(),createElement:()=>new Element(),activeElement:null};
 for(const id of ['origin','restartWaveBtn','victoryRestartWaveBtn','victoryAgainBtn','victoryCloseBtn']){const e=new Element();e.id=id;doc.body.append(e);}doc.activeElement=$('origin');
 ...
 const s={console,Promise,mode:'online',phase:'playing',pausedFrom:'playing',localMatchStats:{},localRoom:{self:8,players:[]},localSurvivalCheckpoint:null,pendingGameConfirmation:null,
  ...
  Net:{expandMachineBullets:()=>[]},cacheMap(){},resize(){},resetOnlineMotion(){s.online.snapshots=[];},closeVictory(){s.closed=(s.closed||0)+1;},
...
test('fresh authoritative restart state closes the menu after wave one and allows another loss result',async()=>{
 ...
 s.receiveOnlineState(statePacket({generation:2,tick:12,phase:'matchOver',status:'lost'}));assert.equal(s.results,1);assert.equal(s.online.lastMatch,2);
});
test('loss results offer wave retry and full replay for spectating hosts of bot squads',async()=>{
 ...
 assert.equal($('victoryRestartWaveBtn').hidden,false);assert.equal($('victoryRestartWaveBtn').textContent,'RESTART WAVE 5');assert.equal($('victoryAgainBtn').hidden,false);assert.equal($('restartWaveBtn').hidden,true);
