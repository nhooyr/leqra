"""v4.6 retained room/browser regression: dark-only theme, callsigns, paint ownership, teams and speed stacks."""
import argparse,json,re,time
from pathlib import Path
from urllib.parse import urlencode
from playwright.sync_api import sync_playwright
ap=argparse.ArgumentParser();ap.add_argument('--url',default='http://127.0.0.1:8455');ap.add_argument('--output',default='test-output/polish46-regression');a=ap.parse_args()
...
def load(b,size=(1365,950),live=False,code=None,name='ALPHA'):
 ...
 p.add_script_tag(content=js);wait(p,'window.__test&&leqra.version==="4.6.0"');p.evaluate('__test.render()');return p
