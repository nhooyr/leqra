...
with sync_playwright() as pw:
 ...
 try:
  ...
  host.locator('#onlineRoomCode').fill('Party Room 420');host.locator('#renameRoomBtn').click();host.wait_for_function('leqra.getState().online.code==="Party Room 420"');friend.wait_for_function('leqra.getState().online.code==="Party Room 420"');home='Party Room 420'
  ...
  buttons=host.evaluate("""()=>{const a=chatBtn.getBoundingClientRect(),b=enemyChatBtn.getBoundingClientRect();return{roomX:a.x,enemyX:b.x,enemyBg:getComputedStyle(enemyChatBtn).backgroundColor,enemyVisible:enemyChatBtn.offsetParent!==null}}""")
  check(buttons['enemyVisible'] and buttons['enemyX']>buttons['roomX'] and buttons['enemyBg'] in ['rgb(159, 37, 50)','rgb(189, 45, 60)','rgb(214, 52, 69)'],'Matchmaking shows a red opponent-chat button to the right of room chat')
  check(friend.evaluate('document.body.scrollWidth<=innerWidth+1'),'320px matchmaking HUD with both chat buttons has no horizontal overflow')
  host.locator('#enemyChatBtn').click();host.locator('#chatInput').fill('Hello enemy side');host.locator('#chatSendBtn').click();host.wait_for_timeout(80);host.locator('#chatCloseBtn').click()
  opp[0].wait(lambda m:m['type']=='chat' and m.get('channel')=='opponent' and m['message']['text']=='Hello enemy side');check(True,'Opponent chat reaches the enemy side over the real server')
  friend.locator('#enemyChatBtn').click();check('Hello enemy side' not in friend.locator('#chatMessages').inner_text(),'Opponent-only message does not leak to sender teammates');friend.locator('#chatCloseBtn').click()
  host.locator('#soundBtn').click();before_notify=host.evaluate('__test.lastChatNotify');opp[0].send(type='chat',channel='opponent',text='Enemy reply');host.wait_for_function("document.querySelector('#enemyChatBadge').textContent==='1'");after_notify=host.evaluate('__test.lastChatNotify');check(host.locator('#chatPanel').is_hidden() and host.locator('#enemyChatBadge').inner_text()=='1' and after_notify>before_notify,'Incoming opponent message marks minimized chat unread and triggers the audio-notification path')
  ...
  check(watch.locator('#enemyChatBtn').is_hidden(),'Pure matchmaking spectators do not get an opponent-chat control without a side')
