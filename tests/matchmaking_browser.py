...
with sync_playwright() as pw:
 ...
 try:
  ...
  host,hc=load(b,'Party Host');host.locator('#addLocalBtn').click();host.locator('#roomP2Callsign').fill('Keyboard Ally');host.locator('#roomP2Callsign').press('Enter');host.locator('#copyInviteBtn').click();host.wait_for_function('leqra.getState().online?.connected');home=host.evaluate('leqra.getState().online.code')
  ...
  check(host.locator('#roomCodeEditor').is_visible() and friend.locator('#roomCodeEditor').is_hidden(),'Only the host can edit the online room code')
  host.locator('#onlineRoomCode').fill('Party Room 420');host.locator('#onlineRoomCode').press('Enter');host.wait_for_function('leqra.getState().online.code==="Party Room 420"');friend.wait_for_function('leqra.getState().online.code==="Party Room 420"');home='Party Room 420'
  check(parse_qs(urlparse(host.evaluate('window.__updatedURL')).query)['room']==[home] and parse_qs(urlparse(friend.evaluate('window.__updatedURL')).query)['room']==[home],'Room rename updates every connected member and invite URL')
  ...
  check(host.locator('#enemyChatBtn').count()==0 and host.locator('#chatBtn').is_visible(),'Matchmaking uses one integrated chat button')
  check(friend.evaluate('document.body.scrollWidth<=innerWidth+1'),'320px matchmaking HUD with integrated chat has no horizontal overflow')
  # One panel receives both channels, while the target switch makes outgoing privacy explicit.
  before_opp=len(opp[0].messages);host.locator('#chatBtn').click();check(host.locator('#chatTitle').inner_text()=='Match chat' and host.locator('#chatConnection').inner_text()=='PARTY + OPPONENT MESSAGES','Integrated matchmaking chat identifies both incoming sources')
  check(host.locator('#chatTargetSwitch').is_visible() and not host.locator('#chatOpponentToggle').is_checked(),'Outgoing chat defaults to the party with a compact opponent toggle')
  host.locator('#chatInput').fill('Party secret');host.locator('#chatSendBtn').click();host.wait_for_timeout(100);host.locator('#chatCloseBtn').click();friend.locator('#chatBtn').click();friend.wait_for_function('document.querySelector("#chatMessages").textContent.includes("Party secret")');check(True,'Party-targeted matchmaking chat reaches the sender party');friend.locator('#chatCloseBtn').click();host.wait_for_timeout(100)
  check(not any(m.get('type')=='chat' and m.get('channel','room')=='room' and m.get('message',{}).get('text')=='Party secret' for m in opp[0].messages[before_opp:]),'Party-targeted chat does not leak to the enemy side')
  host.locator('#chatBtn').click();host.locator('#chatOpponentToggle').check();check(host.locator('#chatOpponentToggle').is_checked() and host.locator('#chatSendBtn').inner_text()=='SEND TO OPPONENT','Compact in-chat toggle switches outgoing messages explicitly to the opponent')
  host.locator('#chatInput').fill('Hello enemy side');host.locator('#chatSendBtn').click();host.wait_for_timeout(80);host.locator('#chatCloseBtn').click()
  opp[0].wait(lambda m:m['type']=='chat' and m.get('channel')=='opponent' and m['message']['text']=='Hello enemy side');check(True,'Opponent-targeted chat reaches the enemy side over the real server')
  friend.locator('#chatBtn').click();check('Hello enemy side' not in friend.locator('#chatMessages').inner_text(),'Opponent-targeted message does not leak to sender teammates');friend.locator('#chatCloseBtn').click()
  host.locator('#soundBtn').click();host.wait_for_timeout(220);before_notify=host.evaluate('__test.lastChatNotify');opp[0].send(type='chat',channel='opponent',text='Enemy reply');host.wait_for_function("document.querySelector('#chatBadge').textContent==='1'");after_notify=host.evaluate('__test.lastChatNotify');check(host.locator('#chatPanel').is_hidden() and host.locator('#chatBadge').inner_text()=='1' and after_notify>before_notify,'Incoming opponent message uses the integrated unread badge and notification path')
  host.locator('#chatBtn').click();host.wait_for_function('document.querySelector("#chatMessages").textContent.includes("Enemy reply")');chat_text=host.locator('#chatMessages').inner_text();check('OPPONENT' in chat_text and 'Enemy reply' in chat_text,'Incoming enemy messages are visibly marked OPPONENT in the shared transcript');host.locator('#chatCloseBtn').click()
  ...
  check(host.locator('#returnRoomBtn').inner_text().strip()=='End match','Online host action is labeled End match')
  ...
  check(watch.locator('#chatBtn').is_hidden(),'Pure matchmaking spectators without a party or side do not get match chat')
  ...
  check(host.locator('#victoryEyebrow').inner_text()=='MATCH RESULTS' and 'CONGRAT' not in host.locator('#victoryDialog').inner_text().upper(),'Results screen avoids congratulating non-winners while still identifying the winning side')
  ...
  check(host.locator('#victoryCloseBtn').bounding_box()['x'] < host.locator('#victoryAgainBtn').bounding_box()['x'],'BACK TO ROOM is left of REMATCH on queued results')
