...
with sync_playwright() as pw:
 ...
 try:
  ...
  host.locator('#addLocalBtn').click();host.wait_for_function('leqra.getState().room.players.length===2');host.locator('#roomRulesBtn').click();host.locator('#rule-mapSize').select_option('giant');host.locator('#rule-mode').select_option('koth');host.locator('#rule-timeLimit').fill('600');host.locator('#rule-scoreTarget').fill('300');host.locator('#rule-pickupRate').select_option('off');host.locator('#applyRulesBtn').click();host.wait_for_function('leqra.getState().rules.mapSize==="giant"')
