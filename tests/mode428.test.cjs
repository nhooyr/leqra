...
function boot({online=false,count=4}={}){
 ...
 for(const name of ['defaultRoomRules','defaultModeRules','currentRules','validateTeamNames','validateRoomRules','activeTeamCount','nextRoomTeam','balanceLocalTeams','normalizeRoomTeams','setLocalRules','isRoomHost','isRoomEditable','roomModeEditable','rulesForRoomMode','clearRoomModePending','paintRoomModeChoice','syncRoomModePicker','rejectRoomMode','selectRoomMode','selectRoomSetting','roomModeKeydown','initRoomModePicker','modeLabel','updateRuleHelp','readRuleTeamSettings','submitRules'])vm.runInContext(declaration(name),s);
