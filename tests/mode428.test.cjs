...
function boot({online=false,count=4}={}){
 ...
 for(const name of ['defaultRoomRules','currentRules','validateTeamNames','validateRoomRules','activeTeamCount','nextRoomTeam','balanceLocalTeams','normalizeRoomTeams','setLocalRules','isRoomHost','isRoomEditable','roomModeEditable','rulesForRoomMode','clearRoomModePending','paintRoomModeChoice','syncRoomModePicker','rejectRoomMode','selectRoomMode','roomModeKeydown','initRoomModePicker','modeLabel','updateRuleHelp','readRuleTeamSettings','submitRules'])vm.runInContext(declaration(name),s);
